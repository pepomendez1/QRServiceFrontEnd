...
export class SvgLibraryService {
  ...
  private preloadedLogo: SafeHtml | null = null; // Cache for preloaded logo
   ...
   */
  preloadLogo(logoUrl: string): Observable<void> {
    if (logoUrl.startsWith('<svg')) {
      // Inline SVG: sanitize and cache it directly
      ...
      this.preloadedLogo = this.sanitizer.bypassSecurityTrustHtml(sanitizedSvg); // Cache the sanitized SVG
      return of(undefined); // Return an Observable<void>
    } else {
      // Fetch and sanitize the logo from a URL
      return this.http.get(logoUrl, { responseType: 'text' }).pipe(
        map((svgContent) => {
          ...
          this.preloadedLogo = sanitizedLogo; // Cache the sanitized logo
        }),
        catchError((error) => {
          console.error(`Error loading logo: ${logoUrl}`, error);
          this.preloadedLogo =
            this.sanitizer.bypassSecurityTrustHtml('<svg></svg>'); // Fallback to empty SVG
          return of(undefined);
        }),
        map(() => undefined) // Ensures the observable is of type void
      );
    }
  }
   ...
   */
  getLogo(): SafeHtml {
    if (!this.preloadedLogo) {
      console.warn('Logo has not been preloaded. Returning empty fallback.');
      return this.sanitizer.bypassSecurityTrustHtml('<svg></svg>'); // Fallback
    }
    return this.preloadedLogo;
  }
