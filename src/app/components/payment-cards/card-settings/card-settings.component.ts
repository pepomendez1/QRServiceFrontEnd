...
import { Cards, PaymentCardsService } from '../payment-cards.service';
...
export class CardSettingsComponent {
  ...
  lostCardAmount: string | null = null;
  lostCardCurrency: string | null = null;
  ...
  ngOnInit(): void {
    this.svgLibrary.getSvg('insurance').subscribe((svgContent) => {
    ...
    this.storeService.getStore().subscribe((store) => {
      this.lostCardAmount = store.init_config?.lost_card_amount || '1500';
      this.lostCardCurrency = store.init_config?.lost_card_currency || '$';
    });
