...
@Directive({
  selector: '[appBiometricValidation]',
})
export class BiometricValidationDirective {
  @Output() verificationFinished = new EventEmitter<void>(); // Emit an event when finished
  @Output() verificationStarted = new EventEmitter<void>(); // Emit an event when finished
  ...
  onStarted({ detail }: any) {
    ...
    this.verificationStarted.emit(); // Emit finished event to the component
  }

  onFinished({ detail }: any) {
    console.log('[METAMAP] User finished verification', detail);
    this.verificationFinished.emit(); // Emit finished event to the component
  }
